...
def find_closest_command(user_input):
    closest_command = difflib.get_close_matches(
        user_input, input_variants, n=1)
    if closest_command:
...
def add_new_contact(*arg, a_book=AddressBook, n_book=Notebook):
    a_book.add_record(Record(name=arg[0].capitalize(
    ), phone=arg[1], birthday=arg[2], email=arg[3], address=arg[4]))
...
def show_contact(*arg, a_book=AddressBook, n_book=Notebook):
...
def show_all(*arg, a_book=AddressBook, n_book=Notebook):
    ...
    sorted_data = sorted(a_book.values(), key=lambda x: x['user_name'])
    counter = 1
    for user_data in sorted_data:
        ...
        for number in phones:
            formatted_number = "{}({})-{}-{}".format(
                number[:4], number[4:6], number[6:8], number[8:])
...
def help_commands(*arg, a_book=AddressBook, n_book=Notebook):
    ...
    for cmd in commands:
        ...
        s2 = "For calling write" + GREEN + f":{cmd['input view'][0]}" + RESET + ',' + GREEN + \
            f" {cmd['input view'][1]}" + RESET + " or " + \
            GREEN + f"{cmd['input view'][2]}" + RESET
...
def sort_files(*arg, a_book=AddressBook, n_book=Notebook):
